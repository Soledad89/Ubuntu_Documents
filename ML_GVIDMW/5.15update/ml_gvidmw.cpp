...
short minisrconppi1[2]={600+85,300+85};//记录PPI开窗中心点坐标
short minisrconppi2[2]={600+85,300+85};

bool bshowmarkonPPI=false;//决定是否绘制PPi标绘层(聚标环和开窗等)；总开关
 ...
 void dispUpdate(int channel,s_ppi_video_for_disp *dispdat,MyWidget*  pmywidget)
{
    ...
    if(wd->ppi->bFirstInfo == true)
    {
        ...
        if(wd->ppi->scopeMode == 0&&bHmode[channel])//H显
        {
            if(bpkg==true)
            {
...
/*  //以下方法只能改变窗口尺寸而已 ，雷达画面不变
 ...
 Status ML_GVID_SetPPIMode(char VideoChannel, short theMode)
 {
    ...
    ML_GVID_ClearPPI(VideoChannel+1);//改变模式先清屏，以防新旧模式交叠
